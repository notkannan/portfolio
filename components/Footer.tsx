  ...
  return (
    <footer className='w-full pt-20 pb-10' id='contact'>
        ...
        <div className="flex flex-col items-center">
            <h1 className='heading lg:max-w-[50vw]'>Let&apos;s build something amazing<span className='text-purple'> together, </span>I&apos;m all ears</h1>
            <p className='text-white-200 md:mt-10 my-5 text-center'>Feel free to reach out to me using the below links, See you there!</p>
        ...
        <div className="flex justify-center mt-5 items-center md:gap-3 gap-6">
          {socialMedia.map((profile) => (
            <Link href={profile.link} target='_blank' key={profile.id} className='w-10 h-10 cursor-pointer flex justify-center items-center backdrop-filter backdrop-blur-lg saturate-180 bg-opacity-75 bg-black-200 rounded-lg border border-black-300'>
              <img src={profile.img} alt='Kannan Karthikeyan' width={20} height={20} />
            </Link>
          ))}
        </div>
